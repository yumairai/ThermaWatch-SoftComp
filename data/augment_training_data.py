...
def main():
    input_path = 'data/Dataset_Master_ERA5_Ready_LSTM.csv'
    output_path = 'data/Dataset_Master_ERA5_Ready_LSTM_training.csv'
    
    if not os.path.exists(input_path):
        print(f"[ERROR] File asli '{input_path}' tidak ditemukan.")
        return
        
    print(f"1. Membaca data asli dari {input_path}...")
    ...
    print(f"2. Menyuntikkan {num_events} event anomali termal buatan...")
    ...
    for _ in range(num_events):
        ...
        for offset, val in enumerate(event_profile):
            ...
            if pd.notna(hist_mean):
                ...
                df_train.at[target_row_idx, 'ERA5_LST_Mean'] = new_temp
                df_train.at[target_row_idx, 'LST_Mean'] = new_temp
                
                # Update juga LST_Max dan Percentile95 agar seimbang
                df_train.at[target_row_idx, 'ERA5_LST_Max'] = new_temp + 2.0
                df_train.at[target_row_idx, 'LST_Max'] = new_temp + 2.0
                df_train.at[target_row_idx, 'ERA5_LST_Percentile95'] = new_temp + 1.2
                df_train.at[target_row_idx, 'LST_Percentile95'] = new_temp + 1.2
                
        events_injected += 1
        
    print(f"   Selesai! Berhasil menyuntikkan {events_injected} event pemanasan.")
    ...
    # Ekspor ke file baru
    print(f"4. Menyimpan dataset latih baru ke '{output_path}'...")
    df_train.to_csv(output_path, index=False)
    
    print("\n=== PEMBUATAN DATASET LATIH AUGMENTASI SUKSES ===")
    print(f"Total Baris Data Latih: {len(df_train)}")
