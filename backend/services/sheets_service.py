...
def get_daily_data(sheet_name="daily_data"):
    """Fungsi helper untuk membaca data harian dari Google Sheets dan memetakan kolomnya (cached)."""
    try:
...
def get_predictions(sheet_name="daily_data"):
    """Fungsi helper untuk membaca data prediksi dari Google Sheets (cached)."""
    try:
        service = SheetsService()
        df = service.read_data(spreadsheet_name="thermawatch-data", sheet_name=sheet_name)
        if df.empty:
